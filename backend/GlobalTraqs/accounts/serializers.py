...
class upVoteStorySerializer(serializers.ModelSerializer):
    title = serializers.CharField(source="pinId.title", read_only=True)
    description = serializers.CharField(source="pinId.description", read_only=True)
    pinAuthor = serializers.SerializerMethodField()
    category = serializers.CharField(source="pinId.category.id", read_only=True)

    class Meta:
        model = upVoteStory
        fields = ['id', 'pinId', 'title', 'description', 'pinAuthor', 'category']

    def get_pinAuthor(self, obj):
        # obj.pinId may be None, or pinId.owner may be None after deletions.
        pin = getattr(obj, "pinId", None)
        if not pin:
            return None
        owner = getattr(pin, "owner", None)
        # return username if owner exists, otherwise None (frontend can show "anonymous")
